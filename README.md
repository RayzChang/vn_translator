...
> 越南語 ↔ 繁體中文翻譯 PWA，用 AI 搞定南北越腔調、男女用語、回譯確認，並自動建立個人詞庫。
...
| **個人詞庫** | 登入後可建立詞庫，翻譯時優先查詞庫，命中則不呼叫 API |
| **自動拆詞存詞庫** | 翻譯時 AI 自動提取單字、詞組、短語並存入詞庫；若詞組可拆出常用單字，會同時存兩筆（例：`ăn KFC` 與 `ăn` 都會存） |
| **詞庫去重** | 相同原文不重複儲存，以資料庫唯一約束 + `ON CONFLICT` 處理 |
| **加入詞庫** | 翻譯結果可手動加入詞庫，自訂譯文 |
| **PWA** | 可加到手機主畫面，離線也能開介面 |
| **多裝置同步** | 登入後金鑰、設定與詞庫同步，公司／家裡／手機都能用 |
